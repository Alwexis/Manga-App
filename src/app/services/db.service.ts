...
export class DbService {
  ...
  //private _DBURL = 'https://tellevoappapi-production.up.railway.app/'
  ...
  */
  get(collection: string, query?: [string]) {
    let url = this._DBURL + collection;
    if (query != undefined) {
      url += '?';
      query.forEach(q => {
        url += q + '&';
   ...
   */
  insertOne(collection: string, data: {}, autoIncrement?: boolean) {
    const url = this._DBURL + collection;
    const body = {
      objectToInsert: data,
  ...
  updateMany(collection: string, query: string, data: {}) {
    let url = this._DBURL + collection + '?' + query;
